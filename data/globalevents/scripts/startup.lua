...
function onStartup()
	...
	if resultId ~= false then
		repeat
			...
			db.asyncQuery('INSERT INTO `account_ban_history` (`account_id`, `reason`, `banned_at`, `expired_at`, `banned_by`) VALUES (' .. accountId .. ', ' .. db.escapeString(result.getString(resultId, 'reason')) .. ', ' .. result.getDataLong(resultId, 'banned_at') .. ', ' .. result.getDataLong(resultId, 'expires_at') .. ', ' .. result.getNumber(resultId, 'banned_by') .. ')')
